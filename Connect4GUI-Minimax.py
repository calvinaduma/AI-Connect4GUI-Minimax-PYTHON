...
###############################################################################
    ...
    def max_value(player, board, depth_limit):
        ...
        for s in successors:
            utility_value = max(utility_value, value(next_player, s[1], depth_limit-1))
    ...
    def min_value(player, board, depth_limit):
        ...
        successors = get_child_boards(player, board)
        for s in successors:
            utility_value = min(utility_value, value(next_player, s[1], depth_limit-1))
        return utility_value

    next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
    score = float('-inf')

    # gets the max score of initial moves max_player can make
    for col_idx in range(board.cols):
        new_board = board.clone()
        if new_board.placeable(col_idx):
            new_board.place(max_player, col_idx)
            new_score = value(next_player, new_board, depth_limit)
            if score < new_score:
                placement = col_idx # update best column
                score = new_score # update best score
...
###############################################################################
    ...
    def exp_value(player, board, depth_limit):
        ...
        successors = get_child_boards(player, board)
        for s in successors:
            probability = 1 / len(get_child_boards(player, board)) # equal probability of choosing a child node
            utility_value += probability * value(next_player, s[1], depth_limit-1)
        return utility_value
    ...
    for col_idx in range(board.cols):
        ...
        if new_board.placeable(col_idx):
            new_board.place(max_player, col_idx)
            new_score = value(max_player, new_board, depth_limit)
            if score < new_score:
                placement = col_idx # update best column
                score = new_score # update best score
